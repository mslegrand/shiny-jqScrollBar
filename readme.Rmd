...
# Steps to Implement *jqScrollBar*


- for arrows, checkout Unicode Character “▾” (U+25BE)
...
svgR(wh=WH,text('STEP 1',xy=c(20,20)),
    box(pos, 1, 'jqScrollBar_svg.R', '#BBFFBB','add to parms', 'edit svgR','commit svg'),
...
svgR(wh=WH, text('STEP 2',xy=c(20,20)),
    ...
    box(pos, 2, 'jqScrollBar.R', 'lightyellow','edit jqScrollBar()'),
    box(pos, 3, 'jqScrollBar.js', 'lightblue','edit jqScrollBar()'),
...
2. In file **jqScrollBar.R** 
    - in function *jqScrollBar()*
...
svgR(wh=WH, text('STEP 3',xy=c(20,20)),
    ...
    box(pos, 2, 'jqScrollBar.R', 'lightyellow','recreate svgTree', 'create & send message'),
    box(pos, 3, 'jqScrollBar.js', 'lightblue','extract value and set element', 'extract and replace tree'),
    box(pos, 4, 'app.R', '#FFCCAA','run to test')
)
```
...
1. In **App.R** 
    - Edit *observeEvent(input\$updateButton* to call
       - *updatejqScrollBar* with a proper value
2. In file **jqScrollBar.R** 
  - In the function **updateJqScrollBar**
    1. Recreate svg Tree (), 
        - this will entail creating the following
            - params=list(ID=inputId, WH=wh, value=value) 
            - svgTree<-*jqScrollBarWrapper*(params=params)
...
```
3. In file **jqScrollBar.js** in function *receivemessage*
    1. extract value(s) 
...
```
4. In **App.R** validate that *updatejqScrollBar* works:
...
svgR(wh=WH,text('STEP 4',xy=c(20,20)),
    box(pos, 1, 'jqScrollBar_svg.R', '#BBFFBB','add mouse events'),
    box(pos, 2, 'app.R', '#FFCCAA','run to test')
)
```

- In file **jqScrollBar_svg.R** 
...
svgR(wh=WH,text('STEP 5',xy=c(20,20)),
    box(pos, 1, 'jqScrollBar.js', 'lightblue','add clicked:'),
    box(pos, 2, 'jqScrollBar.R', 'lightyellow','edit params$CMDS'),
...
2. In **jqScrollBar.R**
    - edit **params\$CMDS** to use cmd in client. 
        - for example to handle a mouse click, add 
          - sprintf('jqScrollBarBinding.clicked("%s", "%s", evt)',params\$ID, mssg)
...
svgR(wh=WH,text('STEP 6',xy=c(20,20)),
    box(pos, 1, 'jqScrollBar.R', 'lightyellow','registerInputHandler'),
    box(pos, 2, 'app.R', '#FFCCAA','run to test')
)
````

1. In file **jqScrollBar.R** 
...
svgR(wh=WH,text('STEP 7',xy=c(20,20)),
    box(pos, 1, 'jqScrollBar.R', 'lightyellow','add to registerInputHandler', 'updatejqScrollBar(...)'),
    box(pos, 2, 'app.R', '#FFCCAA','run to test')
)
